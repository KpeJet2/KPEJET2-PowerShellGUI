...
if (-not (Test-Path $modulePath)) {
    $modulePath = Join-Path $scriptDir 'modules\UserProfileManager.psm1'
}
...
function Update-LogProfile {
    param([string]$ProfileName)
    if ([string]::IsNullOrWhiteSpace($ProfileName)) { return }
    $script:LogProfileName = $ProfileName -replace '[^\w\-]', '_'
    $script:LogDir  = Join-Path $env:TEMP "UPM-Backups\$($script:LogProfileName)"
    $script:LogFile = Join-Path $script:LogDir "$($script:ScriptBaseName)-$($env:COMPUTERNAME)-$($env:USERNAME)-$($script:LogStamp).log"
    Write-UPMLog "Log context switched to profile: $ProfileName"
}
    ...
    try {
        ...
        if ($ErrorRecord) {
            ...
            if ($ErrorRecord.InvocationInfo -and $ErrorRecord.InvocationInfo.PositionMessage) {
                Write-UPMLog "ERR.Position = $($ErrorRecord.InvocationInfo.PositionMessage)" 'ERROR'
            }
        }
    } catch { <# Intentional: non-fatal #> }
...
# ── Capture index (23 operations) ────────────────────────────────────────────
# Each entry maps a tree key to its capture function, data key, sub-tags,
# and flags for save/compare/restore support.
$script:CaptureIndex = [ordered]@{
    'winget'       = @{Idx=1;  Category='Winget Applications';   Function='Get-WingetApplications';   DataKey='WingetApplications'; SubTags=@(); HasSave=$true;  HasCompare=$true;  HasRestore=$true;  Description='All winget-managed applications'; RestoreNote='winget install --id for each missing application'; SaveExample='Get-WingetApplications | Select Name,Id,Version'; RestoreExample='winget install --exact --id \$app.Id --silent' }
    'psenv'        = @{Idx=2;  Category='PowerShell Environment';Function='Get-PSEnvironment';         DataKey='PSEnvironment'; SubTags=@('psver','psmod','psscripts','psprofiles'); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='PS version, modules, profile paths'; RestoreNote='Read-only -- Install-Module for missing modules'; SaveExample='\$PSVersionTable; Get-Module -ListAvailable'; RestoreExample='Install-Module -Name \$m.Name -RequiredVersion \$m.Version -Scope CurrentUser' }
    'userconfig'   = @{Idx=3;  Category='User App Configs';      Function='Get-UserAppConfigs';        DataKey='UserAppConfigs'; SubTags=@('regleys','cfgfiles'); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='HKCU registry keys and %APPDATA% config files'; RestoreNote='Selective registry value restore'; SaveExample='Get-ChildItem HKCU:\\SOFTWARE | Select PSChildName,SubKeyCount'; RestoreExample='Set-ItemProperty -Path \$key.KeyPath -Name \$v.Name -Value \$v.Value' }
    'taskbar'      = @{Idx=4;  Category='Taskbar Layout';        Function='Get-TaskbarLayout';         DataKey='TaskbarLayout'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$true; Description='Pinned items (.lnk), LayoutModification.xml, Taskband blob'; RestoreNote='Writes .lnk files + LayoutModification.xml'; SaveExample='Get-ChildItem \"...\\User Pinned\\TaskBar\" -Filter *.lnk'; RestoreExample='[System.IO.File]::WriteAllBytes(\$pin.Path,[Convert]::FromBase64String(\$pin.LnkBase64))' }
    'printdrivers' = @{Idx=5;  Category='Print Drivers';         Function='Get-PrintDrivers';          DataKey='PrintDrivers'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Installed print drivers with INF path'; RestoreNote='Read-only -- pnputil driver install not automated'; SaveExample='Get-PrinterDriver | Select Name,DriverVersion,PrinterEnvironment'; RestoreExample='pnputil /add-driver \$d.InfPath /install' }
    'mimetypes'    = @{Idx=6;  Category='MIME Types';            Function='Get-MimeTypes';             DataKey='MimeTypes'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$true; Description='File extension to MIME type mappings (HKCU + HKLM)'; RestoreNote='Writes HKCU:\\SOFTWARE\\Classes extension entries'; SaveExample='Get-ChildItem HKCU:\\SOFTWARE\\Classes | Where-Object {\$_.Name -match \"Content Type\"}'; RestoreExample='Set-ItemProperty \"HKCU:\\SOFTWARE\\Classes\\.\$ext\" -Name \"Content Type\" -Value \$mime' }
    'wifi'         = @{Idx=7;  Category='WiFi Profiles';         Function='Get-WiFiProfiles';          DataKey='WiFiProfiles'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='WiFi profile metadata and encrypted XML'; RestoreNote='netsh wlan add profile filename=<xml>'; SaveExample='netsh wlan show profiles | netsh wlan export profile'; RestoreExample='netsh wlan add profile filename=\$xmlPath user=current' }
    'mru'          = @{Idx=8;  Category='Recent Locations MRU';  Function='Get-MRULocations';          DataKey='MRULocations'; SubTags=@('mru_typed','mru_run','mru_opensave','mru_exts'); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='Explorer typed paths, Run history, OpenSave paths, recent extensions'; RestoreNote='Read-only history capture'; SaveExample='Get-ItemProperty HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\TypedPaths'; RestoreExample='N/A -- history entries are read-only captures' }
    'certs'        = @{Idx=9;  Category='Certificates';          Function='Get-CertificateStores';     DataKey='Certificates'; SubTags=@('certs_user','certs_lm'); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='User and Computer certificate store inventory'; RestoreNote='Manual cert import recommended for security'; SaveExample='Get-ChildItem Cert:\\CurrentUser -Recurse | Select Subject,Thumbprint'; RestoreExample='Import-Certificate -FilePath \$cert.ExportPath -CertStoreLocation Cert:\\CurrentUser\\My' }
    'ise'          = @{Idx=10; Category='ISE Configuration';     Function='Get-ISEConfiguration';      DataKey='ISEConfiguration'; SubTags=@('ise_profile','ise_snip','ise_reg','ise_opts','ise_addons','ise_recent'); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='ISE profile, snippets, registry settings, options, add-ons'; RestoreNote='Restores ISE registry + snippet files'; SaveExample='Get-Item HKCU:\\SOFTWARE\\Microsoft\\PowerShell\\*\\PowerShellISE'; RestoreExample='Set-ItemProperty \$r.Name -Value \$r.Value' }
    'terminal'     = @{Idx=11; Category='Terminal Configuration'; Function='Get-TerminalConfiguration'; DataKey='TerminalConfiguration'; SubTags=@('terminal_con','terminal_wt','terminal_profiles'); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Console Host settings, Windows Terminal settings.json, PS profile hashes'; RestoreNote='Writes conhost registry + settings.json'; SaveExample='Get-ItemProperty HKCU:\\Console; Get-Content \$wtSettingsPath'; RestoreExample='Set-ItemProperty HKCU:\\Console -Name \$r.Name -Value \$r.Value' }
    'pshelp'       = @{Idx=12; Category='PS Repositories';       Function='Get-PSHelpRepositories';    DataKey='PSHelpRepositories'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Registered PSRepositories and module paths'; RestoreNote='Register-PSRepository for missing entries'; SaveExample='Get-PSRepository | Select Name,SourceLocation,InstallationPolicy'; RestoreExample='Register-PSRepository -Name \$r.Name -SourceLocation \$r.SourceLocation' }
    'screensaver'  = @{Idx=13; Category='Screensaver Settings';  Function='Get-ScreensaverSettings';   DataKey='ScreensaverSettings'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Screensaver exe, timeout, password, enabled state'; RestoreNote='Writes HKCU Desktop screensaver keys'; SaveExample='Get-ItemProperty \"HKCU:\\Control Panel\\Desktop\"'; RestoreExample='Set-ItemProperty \"HKCU:\\Control Panel\\Desktop\" -Name SCRNSAVE.EXE -Value \$ss.ScreenSaver' }
    'power'        = @{Idx=14; Category='Power Configuration';   Function='Get-PowerConfiguration';    DataKey='PowerConfiguration'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Power plans with active plan identification'; RestoreNote='powercfg /setactive GUID'; SaveExample='powercfg /list'; RestoreExample='powercfg /setactive \$plan.Guid' }
    'display'      = @{Idx=15; Category='Display Layout';        Function='Get-DisplayLayout';         DataKey='DisplayLayout'; SubTags=@('display_dpi','display_pm'); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Monitor resolution, refresh rate, DPI registry, per-monitor DPI'; RestoreNote='DPI registry writes only'; SaveExample='Get-CimInstance Win32_VideoController | Select Name,CurrentHorizontalResolution'; RestoreExample='Set-ItemProperty HKCU:\\Control Panel\\Desktop -Name LogPixels -Value \$dpi' }
    'regional'     = @{Idx=16; Category='Regional & Language';   Function='Get-RegionalSettings';      DataKey='RegionalSettings'; SubTags=@(); HasSave=$true; HasCompare=$true; HasRestore=$false; Description='Culture, locale, home location, installed languages, registry international'; RestoreNote='Read-only -- regional settings are complex to restore'; SaveExample='Get-Culture; Get-WinSystemLocale; Get-WinHomeLocation'; RestoreExample='Set-Culture \$rl.CultureName; Set-WinSystemLocale \$rl.SystemLocale' }
    'envvars'      = @{Idx=17; Category='Environment Variables'; Function='Get-EnvironmentVariables';  DataKey='EnvironmentVariables'; SubTags=@('envvars_user','envvars_machine','envvars_process'); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='User, Machine and Process scope environment variables'; RestoreNote='[Environment]::SetEnvironmentVariable for user scope'; SaveExample='[Environment]::GetEnvironmentVariables([System.EnvironmentVariableTarget]::User)'; RestoreExample='[Environment]::SetEnvironmentVariable(\$v.Name,\$v.Value,\"User\")' }
    'mappeddrives' = @{Idx=18; Category='Mapped Drives';         Function='Get-MappedDrives';          DataKey='MappedDrives'; SubTags=@(); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='Network mapped drives with UNC paths'; RestoreNote='New-SmbMapping for each missing drive'; SaveExample='Get-SmbMapping | Select LocalPath,RemotePath'; RestoreExample='New-SmbMapping -LocalPath \$d.Drive -RemotePath \$d.NetworkPath' }
    'fonts'        = @{Idx=19; Category='Installed Fonts';       Function='Get-InstalledFonts';        DataKey='InstalledFonts'; SubTags=@(); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='System and user font registry inventory'; RestoreNote='Read-only font inventory -- install fonts manually'; SaveExample='Get-ItemProperty \"HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts\"'; RestoreExample='Copy-Item \$fontFile \"C:\\Windows\\Fonts\"; Set-ItemProperty ...Fonts -Name \$f.Name -Value \$f.FileName' }
    'lang'         = @{Idx=20; Category='Language & Speech';     Function='Get-LanguageAndSpeech';     DataKey='LanguageAndSpeech'; SubTags=@('lang_packs','lang_sr','lang_tts','lang_dict','lang_custom'); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='Language packs, speech recognizers, TTS voices, dictionaries'; RestoreNote='Read-only -- language pack install is manual'; SaveExample='Get-WinUserLanguageList; Get-InstalledLanguage'; RestoreExample='Install-Language \$lang.Tag' }
    'quickaccess'  = @{Idx=21; Category='Quick Access Links';    Function='Get-QuickAccessLinks';      DataKey='QuickAccessLinks'; SubTags=@('qa_folders','qa_files','qa_pinned'); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='Frequent folders, recent files, pinned items'; RestoreNote='Shell.Application pin via COM object'; SaveExample='(New-Object -Com Shell.Application).Namespace(\"shell:Recent\").Items()'; RestoreExample='\$shell.Namespace(\$path).Self.InvokeVerb(\"pintohome\")' }
    'explorer'     = @{Idx=22; Category='Explorer Folder View';  Function='Get-ExplorerFolderView';    DataKey='ExplorerFolderView'; SubTags=@('exp_adv','exp_gen','exp_view','exp_bag'); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='Explorer Advanced/General options, view state, Bag MRU'; RestoreNote='HKCU Explorer registry writes'; SaveExample='Get-ItemProperty HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced'; RestoreExample='Set-ItemProperty HKCU:\\...\\Explorer\\Advanced -Name \$r.Name -Value \$r.Value' }
    'search'       = @{Idx=23; Category='Search Providers';      Function='Get-SearchProviders';       DataKey='SearchProviders'; SubTags=@('search_ie','search_ws','search_cor'); HasSave=$true; HasCompare=$false; HasRestore=$false; Description='IE/Edge search scopes, Windows Search settings, Cortana prefs'; RestoreNote='Read-only search configuration capture'; SaveExample='Get-ChildItem \"HKCU:\\SOFTWARE\\Microsoft\\Internet Explorer\\SearchScopes\"'; RestoreExample='N/A -- read-only capture' }
}
...
# Status bar -- 4-part: UPN | Computer | Admin | Status
$statusBar = New-Object System.Windows.Forms.StatusStrip
$statusBar.BackColor    = $clrPanel
$statusBar.SizingGrip   = $false

$statusUPN = New-Object System.Windows.Forms.ToolStripStatusLabel
$statusUPN.Text         = "  $($script:UPNDisplay)  "
$statusUPN.ForeColor    = $clrAccent
$statusUPN.Font         = $fontMain
$statusUPN.BorderSides  = [System.Windows.Forms.ToolStripStatusLabelBorderSides]::Right
$statusUPN.BorderStyle  = [System.Windows.Forms.Border3DStyle]::Etched
$statusUPN.AutoSize     = $true
$statusUPN.ToolTipText  = 'Logged-in UPN'

$statusComputer = New-Object System.Windows.Forms.ToolStripStatusLabel
$statusComputer.Text        = "  $($env:COMPUTERNAME)  "
$statusComputer.ForeColor   = $clrText
$statusComputer.Font        = $fontMain
$statusComputer.BorderSides = [System.Windows.Forms.ToolStripStatusLabelBorderSides]::Right
$statusComputer.BorderStyle = [System.Windows.Forms.Border3DStyle]::Etched
$statusComputer.AutoSize    = $true
$statusComputer.ToolTipText = 'Computer Name'

$statusAdmin = New-Object System.Windows.Forms.ToolStripStatusLabel
$statusAdmin.Text        = if ($script:IsAdmin) { '  ▲ ADMIN  ' } else { '  ● USER  ' }
$fc = if ($script:IsAdmin) { $clrAmber } else { $clrSubtle }
if ($null -ne $fc) { $statusAdmin.ForeColor = $fc }
$statusAdmin.Font        = $fontBold
$statusAdmin.BorderSides = [System.Windows.Forms.ToolStripStatusLabelBorderSides]::Right
$statusAdmin.BorderStyle = [System.Windows.Forms.Border3DStyle]::Etched
$statusAdmin.AutoSize    = $true
$statusAdmin.ToolTipText = if ($script:IsAdmin) { 'Running as Administrator' } else { 'Running as standard user' }

$statusLabel = New-Object System.Windows.Forms.ToolStripStatusLabel
$statusLabel.Text      = 'Ready.'
$statusLabel.ForeColor = $clrText
$statusLabel.Font      = $fontMain
$statusLabel.Spring    = $true   # fills remaining width
$statusLabel.TextAlign = [System.Drawing.ContentAlignment]::MiddleLeft

$statusBar.Items.AddRange([System.Windows.Forms.ToolStripItem[]]@($statusUPN, $statusComputer, $statusAdmin, $statusLabel))
$form.Controls.Add($statusBar)

# ── Menu strip: File | Help ────────────────────────────────────────────────────────────
function New-MenuItem {
    param([string]$Text, [string]$Shortcut = '', [scriptblock]$Action = $null)
    $item = New-Object System.Windows.Forms.ToolStripMenuItem
    $item.Text      = $Text
    $item.ForeColor = $clrText
    $item.BackColor = $clrPanel
    $item.Font      = $fontMain
    if ($Shortcut) {
        try { $item.ShortcutKeys = [System.Windows.Forms.Keys]$Shortcut } catch { <# Intentional: non-fatal #> }
    }
...
$mnuFile.BackColor = $clrPanel
$mnuFile.Font      = $fontMain

$mnuFileCapture = New-MenuItem '&New Capture' '' {
    if ($null -ne $btnCapture) { $btnCapture.PerformClick() }
}
$mnuFileSave = New-MenuItem '&Save Profile...' '' {
    $tabs.SelectedIndex = 1   # switch to SAVE tab
}
$mnuFileOpenStore = New-MenuItem 'Open Profile Store &Folder' '' {
    Start-Process explorer.exe $script:ProfileStorePath
}
$mnuFileOpenLogs = New-MenuItem 'Open &Log Folder' '' {
    $logParent = Join-Path $env:TEMP 'UPM-Backups'
    if (-not (Test-Path $logParent)) { New-Item -ItemType Directory -Path $logParent -Force | Out-Null }
    Start-Process explorer.exe $logParent
}
$mnuFileSep1    = New-Object System.Windows.Forms.ToolStripSeparator
$mnuFileExit    = New-MenuItem 'E&xit' '' { $form.Close() }

$mnuFile.DropDownItems.AddRange([System.Windows.Forms.ToolStripItem[]]@(
    $mnuFileCapture, $mnuFileSave, $mnuFileOpenStore, $mnuFileOpenLogs,
    $mnuFileSep1, $mnuFileExit
))
...
$mnuHelp.BackColor = $clrPanel
$mnuHelp.Font      = $fontMain

$mnuHelpGuide = New-MenuItem '&User Guide (XHTML)' '' {
    $guidePath = Join-Path $scriptDir 'docs\UPM-UserGuide.xhtml'
    if (Test-Path $guidePath) {
        Start-Process $guidePath
    } else {
        [System.Windows.Forms.MessageBox]::Show("User Guide not found at:`n$guidePath",'Guide Not Found',[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null
    }
}

$mnuHelpVersionHistory = New-MenuItem '&Version History' '' {
    $vhPath = Join-Path $scriptDir 'docs\VERSION-HISTORY.md'
    if (Test-Path $vhPath) {
        Start-Process notepad.exe $vhPath
    } else {
        [System.Windows.Forms.MessageBox]::Show("Version history not found at:`n$vhPath",'Not Found',[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null
    }
}

$mnuHelpWhatIf = New-MenuItem '&WhatIf Simulation...' '' {
    Show-WhatIfDialog
}

$mnuHelpSep1  = New-Object System.Windows.Forms.ToolStripSeparator
$mnuHelpAbout = New-MenuItem '&About UPM' '' {
    $msg  = "User Profile Manager (UPM)`n"
    $msg += "Version : 1.1.0`n"
    $msg += "Author  : The Establishment`n`n"
    $msg += "Operations indexed : 23 capture functions`n"
    $msg += "Module  : modules\UserProfileManager.psm1`n"
    $msg += "Profile store : $($script:ProfileStorePath)`n"
    $msg += "Session log   : $($script:LogFile)"
    [System.Windows.Forms.MessageBox]::Show($msg,'About UPM',[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
}

$mnuHelp.DropDownItems.AddRange([System.Windows.Forms.ToolStripItem[]]@(
    $mnuHelpGuide, $mnuHelpVersionHistory, $mnuHelpWhatIf,
    $mnuHelpSep1, $mnuHelpAbout
))

# Assemble menu strip
$menuStrip = New-Object System.Windows.Forms.MenuStrip
$menuStrip.BackColor   = $clrPanel
$menuStrip.ForeColor   = $clrText
$menuStrip.Font        = $fontMain
$menuStrip.RenderMode  = [System.Windows.Forms.ToolStripRenderMode]::Professional
$menuStrip.Items.AddRange([System.Windows.Forms.ToolStripItem[]]@($mnuFile, $mnuHelp))
...
$btnCapture.Add_Click({
    ...
    try {
        ...
        if ($null -ne $script:LiveSnapshot) {
            ...
            Write-UPMLog "Capture complete: $appCount apps, $modCount modules, 23 categories"
            $statusLabel.Text = "Live snapshot ready -- $appCount apps, $modCount modules"
        }
    } catch {
        Write-UPMLog "Capture failed: $_" 'ERROR'
...
$btnSaveProfile.Add_Click({
    ...
    try {
        ...
        Write-UPMLog "Save complete: $outFile"
        ...
        Write-UPMLog "Save failed: $_" 'ERROR'
...
function Load-ProfileList {
    try {
        ...
        $cmpCombo.Tag = $profiles   # store objects for retrieval
...
$btnCmpLoad.Add_Click({
    ...
    try {
        ...
        Write-UPMLog 'Compare diff computed'
...
$btnStartRestore.Add_Click({
    ...
    $btnStartRestore.Enabled = $false
    $restLog.Clear()
    $profName = if ($script:LoadedProfile.Meta.ProfileName) { $script:LoadedProfile.Meta.ProfileName } else { 'unknown' }
    Update-LogProfile $profName
    Write-UPMLog "Restore started: source profile='$profName' options=$($options | ConvertTo-Json -Depth 5 -Compress)"
    ...
    try {
        Invoke-WithProgress $restProgress $restStatusLbl {
            ...
            Write-UPMLog "Rollback saved: $($restResult.RollbackPath)"
            Append-RestLog '' $clrText
            foreach ($log in $restResult.Restored) { Append-RestLog "  ✔ $log" $clrGreen;   Write-UPMLog "Restored: $log" }
            foreach ($log in $restResult.Skipped)  { Append-RestLog "  – $log" $clrSubtle; Write-UPMLog "Skipped: $log" }
            foreach ($log in $restResult.Errors)   { Append-RestLog "  ✘ $log" $clrRed;    Write-UPMLog "Error: $log" 'ERROR' }
            Append-RestLog '' $clrText
            Append-RestLog "Restore complete." $clrAmber
            Write-UPMLog "Restore complete"
            $statusLabel.Text = "Restore complete. Rollback: $($restResult.RollbackPath)"
        }
    } catch {
        Write-UPMLog "Restore FATAL: $_" 'ERROR'
...
# ════════════════════════════════════════════════════════════════════════════
#  WHATIF SIMULATION DIALOG
# ════════════════════════════════════════════════════════════════════════════
function Show-WhatIfDialog {
    # Requires PSWriteHTML / Out-HtmlView. Falls back to a text popup gracefully.
    $wi = New-Object System.Windows.Forms.Form
    $wi.Text            = 'WhatIf Simulation -- UPM Operation Picker'
    $wi.Size            = New-Object System.Drawing.Size(1020, 740)
    $wi.StartPosition   = 'CenterScreen'
    $wi.BackColor       = $clrBg
    $wi.ForeColor       = $clrText
    $wi.Font            = $fontMain
    $wi.FormBorderStyle = 'Sizable'

    # Instructions
    $wiLbl = New-Object System.Windows.Forms.Label
    $wiLbl.Text      = 'Select one or more capture operations. Choose Save or Restore mode, then click  Run -WhatIf  to preview in a browser.'
    $wiLbl.Location  = New-Object System.Drawing.Point(10, 10)
    $wiLbl.Size      = New-Object System.Drawing.Size(990, 20)
    $wiLbl.ForeColor = $clrSubtle
    $wiLbl.Font      = $fontMain
    $wiLbl.BackColor = [System.Drawing.Color]::Transparent
    $wi.Controls.Add($wiLbl)

    # Operation grid
    $wiGrid = New-Object System.Windows.Forms.DataGridView
    $wiGrid.Location               = New-Object System.Drawing.Point(10, 38)
    $wiGrid.Size                   = New-Object System.Drawing.Size(990, 520)
    $wiGrid.BackgroundColor        = $clrCard
    $wiGrid.GridColor              = $clrBorder
    $wiGrid.ForeColor              = $clrText
    $wiGrid.Font                   = $fontMain
    $wiGrid.RowHeadersVisible      = $false
    $wiGrid.AllowUserToAddRows     = $false
    $wiGrid.AllowUserToDeleteRows  = $false
    $wiGrid.ReadOnly               = $false
    $wiGrid.AutoSizeColumnsMode    = 'Fill'
    $wiGrid.SelectionMode          = 'FullRowSelect'
    $wiGrid.MultiSelect            = $true
    $wiGrid.EnableHeadersVisualStyles = $false
    $wiGrid.ColumnHeadersDefaultCellStyle.BackColor = $clrBg
    $wiGrid.ColumnHeadersDefaultCellStyle.ForeColor = $clrText
    $wiGrid.ColumnHeadersDefaultCellStyle.Font      = $fontBold
    $wiGrid.DefaultCellStyle.BackColor  = $clrCard
    $wiGrid.DefaultCellStyle.ForeColor  = $clrText
    $wiGrid.AlternatingRowsDefaultCellStyle.BackColor = [System.Drawing.Color]::FromArgb(45,45,50)

    # Columns: checkbox, idx, category, save/compare/restore flags, save-example, restore-example
    $colSel = New-Object System.Windows.Forms.DataGridViewCheckBoxColumn
    $colSel.HeaderText = 'Select' ; $colSel.Width = 52 ; $colSel.AutoSizeMode = 'None'
    $wiGrid.Columns.Add($colSel) | Out-Null

    foreach ($hdr in @('Idx','Category','Function','Save','Compare','Restore','Save Example','Restore Example')) {
        $c = New-Object System.Windows.Forms.DataGridViewTextBoxColumn
        $c.HeaderText = $hdr ; $c.ReadOnly = $true
        $wiGrid.Columns.Add($c) | Out-Null
    }

    foreach ($kv in $script:CaptureIndex.GetEnumerator()) {
        $e = $kv.Value
        $wiGrid.Rows.Add(
            $false,
            $e.Idx,
            ...
            $e.Function,
            (if ($e.HasSave)    { '✔' } else { '–' }),
            (if ($e.HasCompare) { '✔' } else { '–' }),
            (if ($e.HasRestore) { '✔' } else { '–' }),
            $e.SaveExample,
            $e.RestoreExample
        ) | Out-Null
    }
    $wiGrid.Columns[1].FillWeight = 20
    $wiGrid.Columns[2].FillWeight = 80
    $wiGrid.Columns[3].FillWeight = 80
    $wiGrid.Columns[4].FillWeight = 20
    $wiGrid.Columns[5].FillWeight = 28
    $wiGrid.Columns[6].FillWeight = 28
    $wiGrid.Columns[7].FillWeight = 120
    $wiGrid.Columns[8].FillWeight = 120
    $wi.Controls.Add($wiGrid)

    # Mode radio
    $wiRadioSave    = New-Object System.Windows.Forms.RadioButton
    $wiRadioSave.Text      = 'Show Save Examples'
    $wiRadioSave.Location  = New-Object System.Drawing.Point(10, 572)
    $wiRadioSave.Size      = New-Object System.Drawing.Size(180, 22)
    $wiRadioSave.Checked   = $true
    $wiRadioSave.ForeColor = $clrText
    $wiRadioSave.BackColor = [System.Drawing.Color]::Transparent

    $wiRadioRestore = New-Object System.Windows.Forms.RadioButton
    $wiRadioRestore.Text      = 'Show Restore Examples'
    $wiRadioRestore.Location  = New-Object System.Drawing.Point(200, 572)
    $wiRadioRestore.Size      = New-Object System.Drawing.Size(200, 22)
    $wiRadioRestore.ForeColor = $clrText
    $wiRadioRestore.BackColor = [System.Drawing.Color]::Transparent
    $wi.Controls.Add($wiRadioSave)
    $wi.Controls.Add($wiRadioRestore)

    # Run button
    $wiBtnRun = New-StyledButton 'Run -WhatIf  ▶' 420 566 160 30
    $wi.Controls.Add($wiBtnRun)
    ...
    $wiBtnRun.Add_Click({
        ...
        if ($selectedRows.Count -eq 0) {
            [System.Windows.Forms.MessageBox]::Show('Please check at least one operation row.','No Selection',[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null
            return
        }

        $modeIsSave = $wiRadioSave.Checked
        $modeLabel  = if ($modeIsSave) { 'SAVE' } else { 'RESTORE' }

        # Build HTML
        $htmlParts = [System.Text.StringBuilder]::new()
        [void]$htmlParts.Append(@"
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
<title>UPM WhatIf Simulation -- $modeLabel</title>
<style>
  body  { font-family: Segoe UI, sans-serif; background:#1e1e1e; color:#ddd; margin:24px; }
  h1    { color:#007acc; }
  h2    { color:#b5cea8; border-bottom:1px solid #444; padding-bottom:4px; }
  table { border-collapse:collapse; width:100%; margin-bottom:32px; }
  th    { background:#2d2d2d; color:#007acc; text-align:left; padding:6px 10px; }
  td    { border:1px solid #3c3c3c; padding:6px 10px; vertical-align:top; }
  td.lbl{ color:#9cdcfe; font-weight:bold; width:160px; }
  code  { background:#2d2d2d; color:#ce9178; padding:2px 6px; border-radius:3px; white-space:pre-wrap; }
  .badge-save    { background:#1a4a1a; color:#6db36d; border-radius:3px; padding:1px 6px; }
  .badge-compare { background:#1a3a5a; color:#6daadb; border-radius:3px; padding:1px 6px; }
  .badge-restore { background:#4a2a00; color:#d4a04a; border-radius:3px; padding:1px 6px; }
  .note { color:#888; font-style:italic; }
</style>
</head>
<body>
<h1>UPM WhatIf Simulation -- $modeLabel mode (-WhatIf)</h1>
<p class="note">This is a simulation. No changes will be made. The code blocks show what would run for each selected operation.</p>
<table>
  <tr><th>#</th><th>Category</th><th>Function</th><th>Flags</th><th>Code Example ($modeLabel)</th><th>Restore Note</th></tr>
"@)

        foreach ($rowIdx in $selectedRows) {
            $row = $wiGrid.Rows[$rowIdx]
            $idx      = $row.Cells[1].Value
            $cat      = $row.Cells[2].Value
            $func     = $row.Cells[3].Value
            $hasSave  = $row.Cells[4].Value
            $hasComp  = $row.Cells[5].Value
            $hasRest  = $row.Cells[6].Value
            $saveEx   = $row.Cells[7].Value
            $restEx   = $row.Cells[8].Value

            # Look up full detail from index
            $detail = $script:CaptureIndex.Values | Where-Object { $_.Idx -eq $idx } | Select-Object -First 1
            $codeEx = if ($modeIsSave) { $saveEx } else { $restEx }
            $note   = if ($detail) { $detail.RestoreNote } else { '' }
            $desc   = if ($detail) { $detail.Description } else { '' }

            $badges = ''
            if ($hasSave  -eq '✔') { $badges += '<span class="badge-save">SAVE</span> ' }
            if ($hasComp  -eq '✔') { $badges += '<span class="badge-compare">COMPARE</span> ' }
            if ($hasRest  -eq '✔') { $badges += '<span class="badge-restore">RESTORE</span> ' }

            [void]$htmlParts.Append(@"
  <tr>
    <td>$idx</td>
    <td><strong>$cat</strong><br/><span class="note">$desc</span></td>
    <td><code>$func</code></td>
    <td>$badges</td>
    <td><code>$codeEx</code></td>
    <td class="note">$note</td>
  </tr>
"@)
        }

        [void]$htmlParts.Append('</table></body></html>')

        # Write temp HTML and open
        $tmpHtml = Join-Path $env:TEMP "UPM-WhatIf-$modeLabel-$(Get-Date -Format 'yyyyMMdd-HHmmss').html"
        [System.IO.File]::WriteAllText($tmpHtml, $htmlParts.ToString(), [System.Text.Encoding]::UTF8)
        Write-UPMLog "WhatIf HTML generated: $tmpHtml mode=$modeLabel ops=$($selectedRows.Count)"

        # Try Out-HtmlView (PSWriteHTML) first, fall back to Start-Process
        try {
            if (Get-Command Out-HtmlView -ErrorAction SilentlyContinue) {
                $htmlParts.ToString() | Out-HtmlView -Title "UPM WhatIf - $modeLabel"
            } else {
