...
"""
    ...
    async def run_tests(
        ...
        """
        Main handler: receives handover package (JSON str from Code-B-iSmuth via Workflow edge).
        ...
        """
        if self.monitor:
            self.monitor.mark_received(self.AGENT_ID)
            self.monitor.start_processing(self.AGENT_ID)
        ...
        try:
            ...
            message = ChatMessage(role="user", text=handover_json)
            augmented = [message]
            ...
            if self.monitor:
                self.monitor.mark_sent(self.AGENT_ID)
        except Exception as ex:
            if self.monitor:
                self.monitor.mark_error(self.AGENT_ID, str(ex))
            raise
        finally:
            if self.monitor:
                self.monitor.stop_processing(self.AGENT_ID)
