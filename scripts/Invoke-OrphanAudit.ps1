...
<#
.SYNOPSIS
    Scans workspace for orphaned files not referenced by any script or config.
.PARAMETER WorkspacePath
    Root of the workspace to scan.
.PARAMETER ReportPath
    Path to write the orphan audit report.
#>
...
foreach ($exclude in $exclusions) {
    ...
    foreach ($manifest in $manifests) {
        ...
        Get-ChildItem -Path $folderPath -File -ErrorAction SilentlyContinue | ForEach-Object {
            $fileName = $_.Name
            if ($fileName -like 'PwShGUI-v-*') { return }
            if ($fileName -like 'orphan-audit-*') { return }
            if ($fileName -like 'orphan-audit-core-*') { return }
            if ($fileName -like 'orphan-cleanup-*') { return }
            if ($fileName -like 'report-retention-*') { return }
            if ($fileName -like 'xhtml-triage-*') { return }
