...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Rename Access rule $($RuleName) to $($NewName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Rename-BrokerAccessPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
