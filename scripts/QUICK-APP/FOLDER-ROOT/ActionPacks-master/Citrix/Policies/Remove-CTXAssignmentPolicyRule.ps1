...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Remove Assignment rule $($RuleName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Remove-BrokerAssignmentPolicyRule @cmdArgs | Select-Object *
    $success = $true
    if($SRXEnv) {
