...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Remove Entitlement rule $($RuleName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Remove-BrokerEntitlementPolicyRule @cmdArgs | Select-Object *
    $success = $true
    if($SRXEnv) {
