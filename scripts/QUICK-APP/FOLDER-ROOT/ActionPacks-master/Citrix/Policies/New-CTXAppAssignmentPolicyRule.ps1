...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Create App Assignment rule $($RuleName)" -LoggingID ([ref]$LogID)
    ...
    $ret = New-BrokerAppAssignmentPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
