...
try{ 
    ...
    $rule = Get-BrokerAssignmentPolicyRule @cmdArgs

    StartLogging -ServerAddress $SiteServer -LogText "Set rule metadata $($PropertyName)" -LoggingID ([ref]$LogID)
    ...
    $null = Set-BrokerAssignmentPolicyRuleMetadata @setArgs
    $ret = Get-BrokerAssignmentPolicyRule @cmdArgs | Select-Object -ExpandProperty MetadataMap
    $success = $true
    if($SRXEnv) {
