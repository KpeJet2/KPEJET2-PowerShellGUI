...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Change App Assignment rule $($RuleName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Set-BrokerAppAssignmentPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
