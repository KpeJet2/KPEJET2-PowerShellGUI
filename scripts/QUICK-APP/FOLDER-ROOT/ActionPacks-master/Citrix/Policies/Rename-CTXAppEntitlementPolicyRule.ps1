...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Rename App Entitlement rule $($RuleName) to $($NewName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Rename-BrokerAppEntitlementPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
