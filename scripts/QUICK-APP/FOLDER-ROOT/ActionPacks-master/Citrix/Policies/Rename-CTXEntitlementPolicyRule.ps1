...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Rename Entitlement rule $($RuleName) to $($NewName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Rename-BrokerEntitlementPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
