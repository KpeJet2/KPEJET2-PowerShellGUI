...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Create Assignment rule $($RuleName)" -LoggingID ([ref]$LogID)
    ...
    $ret = New-BrokerAssignmentPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
