...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Create Entitlement rule $($RuleName)" -LoggingID ([ref]$LogID)
    ...
    [hashtable]$cmdArgs = @{'ErrorAction' = 'Stop'
                            ...
                            'Name' = $RuleName
                            'Enabled' = $Enabled
                            'SecureIcaRequired' = $SecureIcaRequired
    ...
    $ret = New-BrokerEntitlementPolicyRule @cmdArgs | Select-Object $Properties
    $success = $true
    if($SRXEnv) {
