...
try{ 
    ...
    $result = Get-AdminService @cmdArgs | Select-Object DNSName,ServiceHostId
    foreach($itm in $result){
        if($SRXEnv){
            $null = $SRXEnv.ResultList.Add($itm.ServiceHostId) # value
            $null = $SRXEnv.ResultList2.Add($itm.DNSName) # display
        }
        else{
