...
<#
    ...
    .Parameter NewUid
        [sr-en] New uid of the Zone 
        [sr-de] Neue Uid der Zone
...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Set Zone $($ZoneName)" -LoggingID ([ref]$LogID)
    ...
    if($PSBoundParameters.ContainsKey('NewUid') -eq $true){
        $cmdArgs.Add('NewUid',$NewUid)
    }
    ...
    $ret = Set-ConfigZone @cmdArgs | Select-Object *
    $success = $true
    if($SRXEnv) {
