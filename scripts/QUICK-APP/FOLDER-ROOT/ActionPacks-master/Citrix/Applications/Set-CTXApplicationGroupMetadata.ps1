...
<#
    ...
    .Parameter GroupName
        ...
        [sr-de] Name des Anwendungsgruppe

    .Parameter GroupUid
        [sr-en] Uid of the application group
        [sr-de] Uid des Anwendungsgruppe
...
try{ 
    ...
    $group = Get-BrokerApplicationGroup @cmdArgs

    StartLogging -ServerAddress $SiteServer -LogText "Set application group metadata $($PropertyName)" -LoggingID ([ref]$LogID)
    $cmdArgs = @{'ErrorAction' = 'Stop'
                'AdminAddress' = $SiteServer
                'ApplicationGroupId' = $group.Uid
                'PassThru' = $null
                'Name' = $PropertyName
                'Value' = $PropertyValue
                'LoggingId' = $LogID
    ...
    $ret = Set-BrokerApplicationGroupMetadata @cmdArgs | Select-Object -ExpandProperty MetadataMap
    $success = $true
    if($SRXEnv) {
