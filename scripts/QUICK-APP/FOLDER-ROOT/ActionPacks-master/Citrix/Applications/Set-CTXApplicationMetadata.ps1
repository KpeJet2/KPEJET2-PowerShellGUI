...
<#
    .SYNOPSIS
        Creates or updates metadata for application
...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Set application metadata $($PropertyName)" -LoggingID ([ref]$LogID)
    ...
    $ret = Set-BrokerApplicationMetadata @cmdArgs | Select-Object -ExpandProperty MetadataMap
    $success = $true
    if($SRXEnv) {
