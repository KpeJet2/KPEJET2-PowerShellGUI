...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Remove Admin folder metadata $($PropertyName)" -LoggingID ([ref]$LogID)
    ...
    $null = Remove-BrokerAdminFolderMetadata @delArgs
    $success = $true
    $ret = Get-BrokerAdminFolder @cmdArgs | Select-Object -ExpandProperty MetadataMap
    if($SRXEnv) {
