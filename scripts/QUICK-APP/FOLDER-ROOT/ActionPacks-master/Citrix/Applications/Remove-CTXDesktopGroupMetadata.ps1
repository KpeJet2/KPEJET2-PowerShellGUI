...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Remove desktop group metadata $($PropertyName)" -LoggingID ([ref]$LogID)
    ...
    $null = Remove-BrokerDesktopGroupMetadata @delArgs
    $success = $true
    $ret = Get-BrokerDesktopGroup @cmdArgs | Select-Object -ExpandProperty MetadataMap
    if($SRXEnv) {
