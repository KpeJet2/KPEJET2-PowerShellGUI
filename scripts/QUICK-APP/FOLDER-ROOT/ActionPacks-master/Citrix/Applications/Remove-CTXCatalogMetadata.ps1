...
try{ 
    ...
    StartLogging -ServerAddress $SiteServer -LogText "Remove catalog metadata $($PropertyName)" -LoggingID ([ref]$LogID)
    ...
    $null = Remove-BrokerCatalogMetadata @delArgs
    $success = $true
    $ret = Get-BrokerCatalog @cmdArgs | Select-Object -ExpandProperty MetadataMap
    if($SRXEnv) {
