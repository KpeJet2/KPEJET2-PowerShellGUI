...
try{
    ...
    [string[]]$PackageTypeFilter = @("None", "Main", "Framework", "Resource", "Bundle", "Xap")
    ...
    if([System.String]::IsNullOrWhiteSpace($ComputerName) -eq $true){
        ...
        if($null -eq $AccessAccount){
            $Script:output = Invoke-Command -ComputerName $ComputerName -ScriptBlock{
                Get-AppxPackage -AllUsers:$true -PackageTypeFilter $Using:PackageTypeFilter -Name '*' `
                    -Publisher '*' -ErrorAction Stop | Select-Object $Using:Properties
            } -ErrorAction Stop                
        }
        else {
