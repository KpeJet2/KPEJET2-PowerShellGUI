...
try{
    if([System.String]::IsNullOrWhiteSpace($ComputerName) -eq $true){
        $null = Mount-AppxVolume -Volume $Volume -Confirm:$false -ErrorAction Stop
    }
    else {
        if($null -eq $AccessAccount){
            $null = Invoke-Command -ComputerName $ComputerName -ScriptBlock{
                Mount-AppxVolume -Volume $Using:Volume -Confirm:$false -ErrorAction Stop
            } -ErrorAction Stop
        }
        else {
    ...
    if($SRXEnv) {
        $SRXEnv.ResultMessage = "Volume $($Volume) mounted"
    }
    else{
        Write-Output "Volume $($Volume) mounted"
    }
}
catch{
    throw
