...
.Parameter Name
    Specifies an name of the user account that disables

.Parameter SID
    Specifies an security ID (SID) of the user account that disables
        ...
        if($null -eq $AccessAccount){
            if($PSCmdlet.ParameterSetName  -eq "ByName"){
                Invoke-Command -ComputerName $ComputerName -ScriptBlock{
                    Disable-LocalUser -Name $Using:Name -Confirm:$false -ErrorAction Stop;
                    Get-LocalUser -Name $Using:Name | Select-Object $Using:Properties
                } -ErrorAction Stop
            }
            else {
                Invoke-Command -ComputerName $ComputerName -ScriptBlock{
                    Disable-LocalUser -SID $Using:SID -Confirm:$false -ErrorAction Stop;
