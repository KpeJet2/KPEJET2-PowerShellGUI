...
try{
    ...
    InvokeUpdateQuery -UpdateQuery $UpdateQuery -UseTransaction:$UseTransaction -Timeout $CommandTimeout -ConnectionName $ConnectionName
    
    if($SRXEnv) {
        $SRXEnv.ResultMessage = $Global:UpdateResult
    }
    else{
        Write-Output $Global:UpdateResult
    }
}
catch{
    throw
