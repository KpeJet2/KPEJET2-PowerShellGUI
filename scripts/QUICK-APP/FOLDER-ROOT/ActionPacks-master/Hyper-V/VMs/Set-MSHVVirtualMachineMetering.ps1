...
try {
    ...
    if($null -ne $Script:VM){
        if($Command -eq 'Reset'){
            Reset-VMResourceMetering -VM $Script:VM -ErrorAction Stop
        }
        elseif($Command -eq 'Disable') {
            Disable-VMResourceMetering -VM $Script:VM -ErrorAction Stop
        } 
        else {
            Enable-VMResourceMetering -VM $Script:VM -ErrorAction Stop
        }
        if($SRXEnv) {
            $SRXEnv.ResultMessage = "Command $($Command) executed on virtual machine $($VMName)"
        }    
        else {
            Write-Output "Command $($Command) executed on virtual machine $($VMName)"
        }
    }
    else{
        if($SRXEnv) {
