...
<#
    .SYNOPSIS
        Gets the BIOS configuration of a virtual machine
        This is only supported on Generation 1 virtual machines
...
try {
    ...
    if($null -ne $Script:VM){
        if($null -eq $AccessAccount){
            $Script:output = Get-VMBios -ComputerName $HostName -VMName $Script:VM.VMName | Select-Object *
        }
        else {
            $Script:output = Get-VMBios -CimSession $Script:Cim -VMName $Script:VM.VMName | Select-Object *
        } 
        if($SRXEnv) {
