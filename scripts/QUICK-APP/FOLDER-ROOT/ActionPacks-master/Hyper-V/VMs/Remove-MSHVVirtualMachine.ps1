...
<#
    ...
    .Parameter VMName
        [sr-en] Name or identifier of the virtual machine to be deleted
#>
...
try {
    ...
    if($null -ne $Script:VM){
        Remove-VM -VM $Script:VM -Force -ErrorAction Stop
        if($SRXEnv) {
            $SRXEnv.ResultMessage = "Virtual machine $($VMName) deleted"
        }    
        else {
            Write-Output "Virtual machine $($VMName) deleted"
        }
    }
    else{
        if($SRXEnv) {
