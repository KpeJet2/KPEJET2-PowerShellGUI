...
try {
    ...
    if($null -ne $Script:VM){
        if($null -eq $AccessAccount){
            $Script:output = Get-VMFirmware -ComputerName $HostName -VMName $Script:VM.VMName | Select-Object *
        }
        else {
            $Script:output = Get-VMFirmware -CimSession $Script:Cim -VMName $Script:VM.VMName | Select-Object *
        } 
        if($SRXEnv) {
