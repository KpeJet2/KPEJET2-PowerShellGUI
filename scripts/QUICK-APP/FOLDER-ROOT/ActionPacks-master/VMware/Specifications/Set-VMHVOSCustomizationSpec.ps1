...
.Parameter NewName
    [sr-en] New name for the OS customization specification
    [sr-de] Neuer Name der Spezifikation
...
try{
    ...
    $spec = Get-OSCustomizationSpec @cmdArgs
    ...
    If($PSBoundParameters.ContainsKey('AdminFullName') -eq $true){
        $cmdArgs.Add('FullName',$AdminFullName)
    }
    ...
    If($PSBoundParameters.ContainsKey('OrgName') -eq $true){
        $cmdArgs.Add('OrgName',$OrgName)
    }
    ...
    $output = Set-OSCustomizationSpec @cmdArgs | Select-Object $Properties
    if($SRXEnv) {
