...
.Parameter PrinterName
    [sr-en] Printer name on which to remove the print job

.Parameter JobID
    [sr-en] ID of a print job to remove on the specified printer

.Parameter ComputerName
    [sr-en] Name of the computer on which to remove the print job
...
try{    
    ...
    if($SRXEnv) {
        ...
        Write-Output "Print job: $($JobID) removed from printer: $($PrinterName) on Computer: $($ComputerName)" 
    }
}
catch{
    throw
