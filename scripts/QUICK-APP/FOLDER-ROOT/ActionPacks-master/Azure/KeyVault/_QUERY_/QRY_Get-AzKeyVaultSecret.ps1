...
try{
    ...
    $ret = Get-AzKeyVaultSecret @cmdArgs | Select-Object $Properties | Sort-Object Name
    foreach($itm in $ret){
        if($null -ne $SRXEnv) {
    ...
    if($IncludeRemoved -eq $true){
        $ret = Get-AzKeyVaultSecret @cmdArgs -InRemovedState | Select-Object $Properties | Sort-Object Name
        foreach($itm in $ret){
            if($null -ne $SRXEnv) {
                $null = $SRXEnv.ResultList.Add($itm.Name)            
