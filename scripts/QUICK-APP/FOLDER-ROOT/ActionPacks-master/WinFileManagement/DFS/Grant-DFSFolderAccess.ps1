...
try{
    ...
    $objRights = Grant-DfsnAccess @cmdArgs | Select-Object $Properties | Sort-Object AccountName
    Invoke-Command -Session $psSes -ScriptBlock{
        param(
            [string]$folder,
            [string]$AccName
        )
        
        dfsutil property sd grant $folder $AccName:RX protect
    } -ArgumentList $FolderPath,$AccountName
