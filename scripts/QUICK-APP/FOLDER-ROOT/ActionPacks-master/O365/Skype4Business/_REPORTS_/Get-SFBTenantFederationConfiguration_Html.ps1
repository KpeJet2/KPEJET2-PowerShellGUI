...
try{
    [string[]]$Properties = @('PSComputerName','AllowedDomains','BlockedDomains','Identity','RunspaceId')
    ConnectS4B -S4BCredential $SFBCredential
    [hashtable]$cmdArgs = @{'ErrorAction' = 'Stop'}      
       
    $result = Get-CsTenantFederationConfiguration @cmdArgs | Select-Object $Properties
