...
try{
    [string[]]$Properties = @('Identity','Description','DefaultCatalogAppsType','GlobalCatalogAppsType','PrivateCatalogAppsType')
    ConnectS4B -S4BCredential $SFBCredential

    [hashtable]$cmdArgs = @{'ErrorAction' = 'Stop'      
                            'Identity' = 'Global'
                            }    
    $result = Get-CsTeamsClientConfiguration @cmdArgs | Select-Object $Properties
