...
try{
    [string[]]$Properties = @('Identity','Description','AllowBroadcastScheduling','AllowBroadcastTranscription','BroadcastAttendeeVisibilityMode','BroadcastRecordingMode')
    ConnectS4B -S4BCredential $SFBCredential

    [hashtable]$cmdArgs = @{'ErrorAction' = 'Stop'
                        'Identity' = 'Global'}      
      
    $result = Get-CsTeamsMeetingBroadcastPolicy @cmdArgs | Select-Object $Properties
