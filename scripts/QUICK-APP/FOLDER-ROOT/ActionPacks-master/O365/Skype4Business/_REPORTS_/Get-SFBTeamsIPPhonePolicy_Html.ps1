...
try{
    [string[]]$Properties = @('Identity','Description','SignInMode','SearchOnCommonAreaPhoneMode','AllowHomeScreen','AllowBetterTogether','AllowHotDesking','HotDeskingIdleTimeoutInMinutes')
    ConnectS4B -S4BCredential $SFBCredential

    [hashtable]$cmdArgs = @{'ErrorAction' = 'Stop'}

    $result = Get-CsTeamsIPPhonePolicy @cmdArgs | Select-Object $Properties
