...
<#
    ...
    .Parameter IncludeResources
        [sr-en] Identities the auto attendant resources whose status is to be retrieved
...
param(    
    ...
    [Parameter(Mandatory = $true)] 
    [string]$Identity,
    [ValidateSet('AudioFile', 'DialByNameVoiceResponses', 'SipProvisioning')]
...
try{
    ...
    if([System.String]::IsNullOrWhiteSpace($IncludeResources) -eq $false){
        $cmdArgs.Add('IncludeResources',$IncludeResources)
    }    
    if([System.String]::IsNullOrWhiteSpace($TenantID) -eq $false){
        $cmdArgs.Add('Tenant',$TenantID)
    }    

    $result = Get-CsAutoAttendantStatus  @cmdArgs | Select-Object *
