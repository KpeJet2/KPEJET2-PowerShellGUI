...
# https://github.com/gwblok/garytown/blob/master/Intune/Update-DellBIOS-Detect.ps1
#>
...
if ($PSCmdlet.ShouldProcess($env:SystemDrive, 'Suspend BitLocker')) {
...
If ($Session -ne $null){
    ...
    if ($PSCmdlet.ShouldProcess($env:COMPUTERNAME, 'Restart in 90 minutes')) {
    ...
    if ($PSCmdlet.ShouldProcess($env:COMPUTERNAME, 'Restart in 5 seconds')) {
