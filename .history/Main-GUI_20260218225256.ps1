...
if (-not $DownloadFolder) { $DownloadFolder = Request-LocalPath -Label "DownloadFolder" -DefaultValue (Join-Path $localRoot "~DOWNLOADS") -TimeoutSeconds 9 }
...
function New-BuildManifest {
    ...
    Get-ChildItem -File -Recurse | Where-Object {
        ...
        $manifestLines += "$rel,`t$($f.CreationTime),`t$($f.LastWriteTime),`t$($f.Length),`t$owner,`t$sha256,`t$motw,`t$cert,`t$compressed,`t$readonly,`t$encrypted"
    }
...
function Test-VersionTag {
    ...
    Get-ChildItem -File -Recurse | Where-Object {
        ...
        $file = $_
        if ($file.Extension -ieq '.json' -or $file.Extension -ieq '.exe' -or $file.Extension -ieq '.dll' -or $file.Extension -ieq '.html' -or $file.Extension -ieq '.htm') { return }
    ...
    foreach ($ex in $excludes) {
        ...
        Get-ChildItem -Path $folderPath -Filter *.txt -File | ForEach-Object {
            ...
            Get-ChildItem -Path $folderPath -File | ForEach-Object {
                $relname = $_.Name
                if ($listed.ContainsKey($relname)) {
                    $size = $_.Length.ToString()
                    if ($size -ne $listed[$relname]) {
                        Write-AppLog "$relname size mismatch (manifest $($listed[$relname]) actual $size)" "Error"
                        if ($diffs) { $diffs.Value += "$folderPath\$relname - size mismatch: manifest $($listed[$relname]) actual $size" }
                        if ($xmlDoc) {
                            ...
                            $fileNode.SetAttribute('issue','size mismatch')
                            $fileNode.SetAttribute('manifest',$listed[$relname])
                            $fileNode.SetAttribute('actual',$size)
                            $folderNode.AppendChild($fileNode) | Out-Null
                        }
                    } else {
                        Write-AppLog "$relname matched" "Success"
                    }
                } else {
...
# Parse the XML diff file to determine if there are mismatches
$versionInfo = Get-VersionInfo
$major = $versionInfo.Major
$minor = $versionInfo.Minor
$build = $versionInfo.Build
