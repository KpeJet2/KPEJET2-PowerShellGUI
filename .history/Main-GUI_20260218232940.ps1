...
param(
    [ValidateSet('quick_jnr', 'slow_snr')]
...
function Request-LocalPathsUnified {
    ...
    foreach ($fieldName in $fieldOrder) {
        ...
        $browseButton.Add_Click({
            ...
            $dialog.Description = "Select folder for $fieldName"
            $dialog.SelectedPath = if ([string]::IsNullOrWhiteSpace($textBox.Text)) { [string]$Defaults[$fieldName] } else { $textBox.Text }
    ...
    foreach ($fieldName in $fieldOrder) {
        ...
        if (-not $timedOut -and $dialogResult -eq [System.Windows.Forms.DialogResult]::OK -and -not [string]::IsNullOrWhiteSpace($entered)) {
