...
"""
    ...
    def __init__(self, client: AzureAIChatClient, log: LogManager, id: str = AGENT_ID):
        self.log = log
        self.agent = client.as_agent(name=self.AGENT_ID, instructions=self.SYSTEM_PROMPT)
    ...
    async def review(
        ...
        self.log.log_prompt(self.AGENT_ID, "\n".join(m.text or "" for m in messages), output)
...
"""
    ...
    @handler
    async def triage(
        self, messages: list[ChatMessage], ctx: WorkflowContext[Never, dict]
    ) -> None:
        response = await self.agent.run(messages)
        output = response.text or ""
...
"""
    ...
    def __init__(self, client: AzureAIChatClient, log: LogManager,
