...
function Show-ConfigMaintenanceForm {
    ...
    foreach ($key in $CurrentPaths.Keys | Sort-Object) {
        ...
        $null = $dgv.Rows.Add($false, $key, $path, $(if($exists){"✓"}else{"✗"}), $fileCount, $sizeMB, $canNest)
    }
    ...
    $nestButton.Add_Click({
        ...
        foreach ($row in $dgv.Rows) {
            if ($row.Cells["Select"].Value -eq $true -and $row.Cells["CanNest"].Value -eq "Yes") {
                $selected += $row.Cells["ConfigKey"].Value
            }
        }
        ...
        $confirmMsg = "Nest $($selected.Count) folder(s) into DefaultFolder?`n`nSelected: $($selected -join ', ')`n`nThis will move folder contents and update config."
        ...
        if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
            Write-AppLog "User confirmed nesting $($selected.Count) folders" "Event"
            [System.Windows.Forms.MessageBox]::Show("Nest operation not yet implemented (requires config XML update logic).", "Not Implemented", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        }
    })
    ...
    $moveButton.Add_Click({
        ...
        if ($folderBrowser.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            ...
            $confirmMsg = "Move content from $($selected.Count) folder(s) to:`n$dest`n`nSource folders will be emptied. Continue?"
            ...
            if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-AppLog "User confirmed move operation to $dest for $($selected.Count) folders" "Event"
                [System.Windows.Forms.MessageBox]::Show("Move operation not yet implemented.", "Not Implemented")
            }
        }
    })
    ...
    $copyButton.Add_Click({
        ...
        if ($folderBrowser.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            $dest = $folderBrowser.SelectedPath
            $confirmMsg = "Copy content from $($selected.Count) folder(s) to:`n$dest`n`nSource folders remain unchanged. Continue?"
            ...
            if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-AppLog "User confirmed copy operation to $dest for $($selected.Count) folders" "Event"
                [System.Windows.Forms.MessageBox]::Show("Copy operation not yet implemented.", "Not Implemented")
            }
        }
    })
    ...
    $abandonButton.Add_Click({
        ...
        if ($confirm1 -eq [System.Windows.Forms.DialogResult]::Yes) {
            $confirm2 = [System.Windows.Forms.MessageBox]::Show("Are you SURE you want to abandon these folders?`n`n(Config will be reset)", "Final Confirmation", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Warning)
            if ($confirm2 -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-AppLog "User confirmed abandon operation for $($selected.Count) folders" "Event"
                [System.Windows.Forms.MessageBox]::Show("Abandon operation not yet implemented.", "Not Implemented")
            }
        }
    })
    ...
    $archiveInPlaceButton.Add_Click({
        $selected = @()
        foreach ($row in $dgv.Rows) {
            if ($row.Cells["Select"].Value -eq $true -and $row.Cells["Exists"].Value -eq "✓") {
                $selected += @{Key=$row.Cells["ConfigKey"].Value; Path=$row.Cells["Path"].Value}
            }
        }
        if ($selected.Count -eq 0) {
        ...
        if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
            Write-AppLog "User confirmed archive in-place for $($selected.Count) folders" "Event"
            [System.Windows.Forms.MessageBox]::Show("Archive in-place operation not yet implemented.", "Not Implemented")
        }
    })
    ...
    $archiveOutYonderButton.Add_Click({
        ...
        if ($folderBrowser.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            ...
            if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-AppLog "User confirmed archive out-yonder to $dest for $($selected.Count) folders" "Event"
                [System.Windows.Forms.MessageBox]::Show("Archive out-yonder operation not yet implemented.", "Not Implemented")
            }
        }
    })
    ...
    $clearCleanButton.Add_Click({
        ...
        foreach ($row in $dgv.Rows) {
            if ($row.Cells["Select"].Value -eq $true -and $row.Cells["Exists"].Value -eq "✓") {
                $selected += @{Key=$row.Cells["ConfigKey"].Value; Path=$row.Cells["Path"].Value; Files=$row.Cells["Files"].Value}
            }
        }
        if ($selected.Count -eq 0) {
            [System.Windows.Forms.MessageBox]::Show("No existing folders selected.", "Info")
        ...
        if ($confirm1 -eq [System.Windows.Forms.DialogResult]::Yes) {
            $confirm2 = [System.Windows.Forms.MessageBox]::Show("FINAL CONFIRMATION`n`nYou are about to DELETE $totalFiles file(s) permanently.`n`nType YES in the confirmation.", "Final Confirmation", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Stop)
            if ($confirm2 -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-AppLog "User confirmed clear-clean for $($selected.Count) folders ($totalFiles files)" "Warning"
                [System.Windows.Forms.MessageBox]::Show("Clear-clean operation not yet implemented.", "Not Implemented")
            }
        }
    })
    ...
    $exportButton.Add_Click({
        ...
        if ($saveDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            ...
            [System.Windows.Forms.MessageBox]::Show("Export operation not yet implemented.", "Not Implemented")
        }
    })
    ...
    $importButton.Add_Click({
        ...
        if ($openDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            ...
            if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
                Write-AppLog "Config import from: $($openDialog.FileName)" "Info"
                [System.Windows.Forms.MessageBox]::Show("Import operation not yet implemented.", "Not Implemented")
            }
        }
    })
    ...
    $resetButton.Add_Click({
        ...
        if ($confirm -eq [System.Windows.Forms.DialogResult]::Yes) {
            Write-AppLog "User confirmed config reset" "Warning"
            [System.Windows.Forms.MessageBox]::Show("Reset operation not yet implemented.", "Not Implemented")
        }
    })
    ...
    $closeButton.Location = New-Object System.Drawing.Point(750, 605)
